...
<?php
    ...
    if (isset($_GET['edit'])) {
        ...
        $DAO = new Database();
        $id = $_GET['edit'];
        $d = $DAO->RetriveSingle("SELECT * FROM `manufacturers` WHERE id = $id");
        $sa = '';
        $su = '';
            ...
            <form action="controller/manage_brand.php" method="post" class="row">
                <div class="col-md-7 col-12 px-3">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="brand_name" placeholder="Enter Product Name" value="<?php if($exits){ echo $d['name'];}?>">
                    </div>
                        <?php
                            if($exits){
                                echo '<input type="hidden" name="brand_id" value="'.$d['id'].'">';
                            }
                        ?>
          
                    <div class="mb-3">
                        <label for="name" class="form-label">Brand Description</label>
                        <textarea name="brand_desc" class="form-control" id="" cols="30" rows="5"><?php if($exits){ echo $d['description'];}?></textarea>
                        </div>
                ...
                <div class="col-md-5 col-12 px-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="brand_status" value="Available" id="flexRadioDefault1" <?php if($exits){  echo $sa;} else echo 'checked'; ?>>
                            <label class="form-check-label" for="flexRadioDefault1">Available</label>
                          </div>
                          <div class="form-check">
                            <input class="form-check-input" type="radio" name="brand_status" value="Unavailable" id="flexRadioDefault2" <?php if($exits){  echo $su;} ?>>
